...
// get a single character from the user, and discard the rest
...
int getChoiceXO(void) {
	...
	if (isalpha(player)) {	// get a character from stdin
		...
		if (player == 'x') {
			...
			printf("Please enter an 'x' or an 'o': ");
			return -1;
		}
		return 0;
	}
...
int getCoords() {
	if (boardPlayable()) {	
		int playerChoice = getUserInput() - '0';
		while (board[playerChoice-1] == 'o' || board[playerChoice-1] == 'x' ||
		playerChoice < 0 || playerChoice > 8) {
			printf("Invalid input. Please try again: ");
			playerChoice = getUserInput() - '0';	// get input and convert to int
		}
